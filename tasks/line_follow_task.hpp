...
void lineFollowTask(void* params)
{
    ...
    while (true)
    {
        ...
        tempLeftSpeed = std::max((int32_t) 0, std::min((int32_t) Motors::MAX_MOTOR_SPEED, tempLeftSpeed));
        tempRightSpeed = std::max((int32_t) 0, std::min((int32_t) Motors::MAX_MOTOR_SPEED, tempRightSpeed));
    
        if (LOGGING_ENABLE)
            LOGI(TAG, "Error: %d, Correction: %d, Speed L:%u R:%u",
                error, correction, (uint16_t) tempLeftSpeed, (uint16_t) tempRightSpeed);
